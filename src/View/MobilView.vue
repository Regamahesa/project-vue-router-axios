<template>
    ...
    <form @submit.prevent="add"  ><br/><br/>
        ...
        <input type="text" v-model="form.image" required><br><br /> <button type="submit" v-show="!updateSubmit">add <i class="fas fa-search-plus"></i></button>
...
export default {
 ...
 update(form){
        return axios.put('http://localhost:3000/mobils/' + form.id , {merk: this.form.merk, tipe: this.form.tipe, bahan: this.form.bahan, harga: this.form.harga, image: this.form.image}).then(res => {
        this.load()
        this.form.id = ''
        this.form.merk = ''
        this.form.tipe = ''
